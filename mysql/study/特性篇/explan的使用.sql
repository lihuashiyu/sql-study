...
-- unique_subquery 是针对在一些包含in子查询的查询语句中，如果查询优化器决定将 in 子查询转换为 exists 子查询，
...
-- 对于单表查询来说，这个 filtered 列的值没什么意义，我们更关注在连接查询
...
-- 当查询列表处有min或者max聚合函数，但是并没有符合 where 子句中的搜索条件的记录时，将会提示该额外信息
...
-- 我们最好能使用索引来替代掉使用临时表；比如：扫描指定的索引 idx_key1 即可
